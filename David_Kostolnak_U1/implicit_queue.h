...
#include "queue.h"
#include "../Semestralna_praca/array.h"
...
namespace structures
{
	...
	inline ImplicitQueue<T>& ImplicitQueue<T>::operator=(const ImplicitQueue<T>& other)
	{
		...
		{
			if (other.size() > size_)
				throw std::logic_error("Fronts are not same size");
			if (other.startIndex_ + other.size_ < other.array_->size())
				Array<T>::copy(*other.array_, startIndex_, *array_, 0, size_);
			else {
				int pocet = (array_->size() - startIndex_);
				Array<T>::copy(*other.array_, other.startIndex_, *array_, 0, pocet);
				Array<T>::copy(*other.array_, 0, *array_, pocet, size_ - pocet);
			}
			startIndex_ = 0;
			size_ = other.size_;
		}
	...
	inline void ImplicitQueue<T>::push(const T& data)
	{
		...
		int pushIndex = (startIndex_ + size_) % array_->size();
		(*array_)[pushIndex] = data;
		size_++;
	}
